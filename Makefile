...
lint-templates :
	$(XML_LINTER) $(PROJECT_SERVER_TEMPLATES_DIR)/WOApplication.xctemplate/TemplateInfo.plist
	$(XML_LINTER) $(FILE_SERVER_TEMPLATES_DIR)/WOComponent.xctemplate/TemplateInfo.plist
	$(XML_LINTER) $(FILE_SERVER_TEMPLATES_DIR)/WODirectAction.xctemplate/TemplateInfo.plist
...
install-project-server-templates : uninstall-project-server-templates
	...
	$(INSTALL_R) \
		$(PROJECT_SERVER_TEMPLATES_DIR)/WOApplication.xctemplate \
		$(XCODE_TEMPLATE_PROJECT_SERVER_SOURCE_DIR)/

uninstall-project-server-templates :
	$(UNINSTALL_R) \
		$(XCODE_TEMPLATE_PROJECT_SERVER_SOURCE_DIR)/WOApplication.xctemplate

install-file-server-templates : uninstall-file-server-templates
	$(MKDIR_P) $(XCODE_TEMPLATE_FILE_SERVER_SOURCE_DIR)
	$(INSTALL_R) \
		$(FILE_SERVER_TEMPLATES_DIR)/WOComponent.xctemplate \
		$(XCODE_TEMPLATE_FILE_SERVER_SOURCE_DIR)/
	$(INSTALL_R) \
		$(FILE_SERVER_TEMPLATES_DIR)/WODirectAction.xctemplate \
		$(XCODE_TEMPLATE_FILE_SERVER_SOURCE_DIR)/

uninstall-file-server-templates :
	$(UNINSTALL_R) \
		$(XCODE_TEMPLATE_FILE_SERVER_SOURCE_DIR)/WOComponent.xctemplate
	$(UNINSTALL_R) \
		$(XCODE_TEMPLATE_FILE_SERVER_SOURCE_DIR)/WODirectAction.xctemplate
